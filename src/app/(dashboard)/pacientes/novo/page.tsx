        ...
        <CardContent>
          <form action={formAction} className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="cpf" className="text-foreground/70">CPF</Label>
                <Input
                  id="cpf" name="cpf"
              ...
              <div className="space-y-2">
                <Label htmlFor="data_nascimento" className="text-foreground/70">Data de Nascimento *</Label>
                <Input id="data_nascimento" name="data_nascimento" type="date" defaultValue={paciente?.data_nascimento ?? ""} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="sexo" className="text-foreground/70">Sexo</Label>
                <select
                  id="sexo" name="sexo"
              ...
              <div className="space-y-2">
                <Label htmlFor="telefone" className="text-foreground/70">Telefone (WhatsApp) *</Label>
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-4 md:grid-cols-4">
                <div className="space-y-2">
                  <Label htmlFor="cep" className="text-foreground/70 flex items-center gap-1.5">
                    CEP
                ...
                <div className="space-y-2 md:col-span-2">
                  <Label htmlFor="logradouro" className="text-foreground/70">Logradouro</Label>
                  <Input ref={logradouroRef} id="logradouro" name="logradouro" defaultValue={paciente?.logradouro ?? ""} placeholder="Rua, Avenida..." />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="numero" className="text-foreground/70">Número</Label>
                  <Input id="numero" name="numero" defaultValue={paciente?.numero ?? ""} placeholder="Nº" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="bairro" className="text-foreground/70">Bairro</Label>
                  <Input ref={bairroRef} id="bairro" name="bairro" defaultValue={paciente?.bairro ?? ""} placeholder="Bairro" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="cidade" className="text-foreground/70">Cidade</Label>
                  <Input ref={cidadeRef} id="cidade" name="cidade" defaultValue={paciente?.cidade ?? ""} placeholder="Cidade" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="uf" className="text-foreground/70">UF</Label>
                  <Input ref={ufRef} id="uf" name="uf" defaultValue={paciente?.uf ?? ""} placeholder="SP" maxLength={2} />
                </div>
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="responsavel" className="text-foreground/70">Responsável Legal</Label>
              <Input id="responsavel" name="responsavel" defaultValue={paciente?.responsavel_legal ?? ""} placeholder="Para menores de idade" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="observacoes" className="text-foreground/70">Observações Críticas</Label>
              <textarea
                id="observacoes"
                name="observacoes"
                defaultValue={paciente?.observacoes_criticas ?? ""}
                className="flex min-h-[80px] w-full rounded-xl border border-border bg-card px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/70 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
